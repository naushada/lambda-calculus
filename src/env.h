...
/* Bind `name` to `term`.  Takes ownership of both.  Rebinding a name replaces
 * the previous definition; terms already built from it are unaffected. */
void  env_define(char *name, Node *term);

/* Replace free occurrences of defined names.  Returns a new tree. */
Node *env_expand(const Node *n);
